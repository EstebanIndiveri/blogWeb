...
function getPosts(req,res){
    ...
    Post.paginate({},options,(err,postsStored)=>{
        if(err){
            res.status(500).send({code:500,message:'Error del servido'})
        }else{
            if(!postsStored){
...
function deletePost(req,res){
    ...
    Post.findByIdAndRemove(id,(err,postDeleted)=>{
        if(err){
            ...
            if(!postDeleted){
                res.status(404).send({code:404,message:'No se pudo eliminar el Post'})
            }else{
...
function getPost(req,res){
    ...
    Post.findOne({url},(err,postStored)=>{
        if(err){
            ...
            if(!postStored){
                res.status(404).send({code:404,message:'No se pudo encontrar el Post'})
            }else{
